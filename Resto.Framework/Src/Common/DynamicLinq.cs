...
#pragma warning restore 0219
        ...
        Expression ParseIntegerLiteral()
        {
            ...
            if (text[0] != '-')
            {
                if (!ulong.TryParse(text, NumberStyles.Integer, formatInfo, out var value))
            ...
            {
                if (!long.TryParse(text, NumberStyles.Integer, formatInfo, out var value))
                    throw ParseError(Res.InvalidIntegerLiteral, text);
                NextToken();
        ...
        Expression ParseRealLiteral()
        {
            ...
            if (last == 'F' || last == 'f')
            {
                if (float.TryParse(text.Substring(0, text.Length - 1), NumberStyles.Float | NumberStyles.AllowThousands, formatInfo, out var f)) value = f;
            }
            else
            {
                if (double.TryParse(text, NumberStyles.Float | NumberStyles.AllowThousands, formatInfo, out var d)) value = d;
            }
        ...
        Expression[] ParseArgumentList()
        {
            ValidateToken(TokenId.OpenParen, Res.OpenParenExpected);
            NextToken();
            var args = token.id != TokenId.CloseParen ? ParseArguments() : Array.Empty<Expression>();
            ValidateToken(TokenId.CloseParen, Res.CloseParenOrCommaExpected);
            NextToken();
            return args;
        }
        ...
        static object ParseNumber(string text, Type type)
        {
            ...
            {
                case TypeCode.SByte:
                    if (sbyte.TryParse(text, NumberStyles.Integer, formatInfo, out var sb)) return sb;
                    break;
                case TypeCode.Byte:
                    if (byte.TryParse(text, NumberStyles.Integer, formatInfo, out var b)) return b;
                    break;
                case TypeCode.Int16:
                    if (short.TryParse(text, NumberStyles.Integer, formatInfo, out var s)) return s;
                    break;
                case TypeCode.UInt16:
                    if (ushort.TryParse(text, NumberStyles.Integer, formatInfo, out var us)) return us;
                    break;
                case TypeCode.Int32:
                    if (int.TryParse(text, NumberStyles.Integer, formatInfo, out var i)) return i;
                    break;
                case TypeCode.UInt32:
                    if (uint.TryParse(text, NumberStyles.Integer, formatInfo, out var ui)) return ui;
                    break;
                case TypeCode.Int64:
                    if (long.TryParse(text, NumberStyles.Integer, formatInfo, out var l)) return l;
                    break;
                case TypeCode.UInt64:
                    if (ulong.TryParse(text, NumberStyles.Integer, formatInfo, out var ul)) return ul;
                    break;
                case TypeCode.Single:
                    if (float.TryParse(text, NumberStyles.Float | NumberStyles.AllowThousands, formatInfo, out var f)) return f;
                    break;
                case TypeCode.Double:
                    if (double.TryParse(text, NumberStyles.Float | NumberStyles.AllowThousands, formatInfo, out var d)) return d;
                    break;
                case TypeCode.Decimal:
                    if (decimal.TryParse(text, NumberStyles.Number, formatInfo, out var e)) return e;
                    break;
            }
            return null;
        }
